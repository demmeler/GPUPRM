...
In the project, the probabilistic roadmap algorithm is applied  to create collision-free trajectories of a robot arm.
...
In order to overlap computation time of GPU and CPU, an asynchronous version of the indicator function was written. Here the function is split in everything until the kernel launch and a wait function, which waits until the kernel finishes and receives the result data. 
...
The solver builds in the first phase the roadmap graph from both the start and the end node, until it is connected.
It is always ensured that the graph consists of two connected components containing the start and the end.
With this, it can be easily determined at each time, if the whole graph has become connected.

At the end a shortest path from the start to the end node is searched by the Dijkstra algorithm.
It has shown that this part a neglectable amount of time compared to the graph building phase.
Therefore it is not parallelized here and the focus lies on building the roadmap.
...
In the first approach, that was evaluated, every process generates new nodes independently from each other.
Then the possible neighbors are determined and the connections, which have to be tested.
The indicator function is called and returns the edges.
